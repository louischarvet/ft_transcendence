import { AdvancedDynamicTexture, Button, Control, Rectangle, TextBlock, ScrollViewer } from "@babylonjs/gui";
...
export default class BjGui {
    ...
    dealersCardsValue;
    cardsInteractions;
    constructor(width, height, sceneFunctions) {
        ...
        window.addEventListener('bj:dealPlace', (e) => {
            const { card, place, placeCardsValue, onSplit } = e.detail || {};
            this.sceneFunctions.dealPlace?.(card, place, onSplit);
            if (placeCardsValue)
                this.setPlaceCardsValue(place, placeCardsValue);
        });
        window.addEventListener('bj:cardInteraction', (e) => {
            const { place } = e.detail || {};
            if (place)
                this.sceneFunctions.setCameraToPlace?.(place);
        });
        window.addEventListener('bj:turnDealerCard', (e) => {
            this.sceneFunctions.turnDealerCard?.();
            const val = e.detail?.cardsValue;
            if (val)
                this.setPlaceCardsValue('dealers', val);
        });
        window.addEventListener('bj:popUpBJ', (e) => {
            const { place } = e.detail || {};
            console.log('Blackjack!', place);
        });
        window.addEventListener('bj:popUpBust', (e) => {
            const { place } = e.detail || {};
            console.log('Bust!', place);
        });
        window.addEventListener('bj:endRound', (e) => {
            ...
            setTimeout(async () => {
                ...
                this.totalBetAmount = 0;
                this.ui.getControlByName("TotalBetLabel").text = `Total Bet: 0 €`;
...
Gain classique : 1,5 fois la mise.";
    ...
    createCoinButton(value, position) {
    ...
    createBetButton() {
        ...
        button.onPointerClickObservable.add(() => {
            ...
            this.ui.getControlByName("BankLabel").text = `Bank: ${this.bankAmount} €`;
            Object.values(this.Bet.areas).forEach(area => {
                if (area.bet > 0) {
            ...
            try {
                ...
                Object.values(this.Bet.areas).forEach(area => {
                    if (area.bet > 0)
                        places[area.place] = { bet: area.bet };
                });
                const playerId = window.BJ_PLAYER_ID || 'player_1';
    ...
    createArea(place) {
        ...
        selectBox.onPointerClickObservable.add(() => {
            ...
            this.ui.getControlByName("TotalBetLabel").text = `Total Bet: ${this.totalBetAmount} €`;
        });
    ...
    constGuiVisibility(isVisible) {
        this.ui.getControlByName("BankLabel").isVisible = isVisible;
        this.ui.getControlByName("TotalBetLabel").isVisible = isVisible;
        this.ui.getControlByName("helpButton").isVisible = isVisible;
        this.ui.getControlByName("quitButton").isVisible = isVisible;
        this.ui.getControlByName("volumeButton").isVisible = isVisible;
    }
    betGuiVisibility(isVisible) {
    ...
    setPlaceCardsValue(place, value) {
    ...
    cardsInteractionsVisibility(isVisible) {
