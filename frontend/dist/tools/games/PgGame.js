...
export default class PgGame {
    ...
    frontAddedObjects = {};
    constructor(keys, sceneFunctions, frontAddedObjects = {}) {
        this.keys = keys;
    ...
    async gameLoop(timer) {
        // Move IA every 1000ms
        if (timer % 1000 < this.fps) {
            if (this.gameMode?.aiMode1) {
                this.aiMovement("Left", this.currentSpeed, this.ball, new Victor(this.paddleLeft.position.x + (this.paddleLeft.width / 2), this.paddleLeft.position.y + (this.paddleLeft.height / 2)), this.gameMode.aiMode1);
            }
            if (this.gameMode?.aiMode2) {
                this.aiMovement("Right", this.currentSpeed, this.ball, new Victor(this.paddleRight.position.x + (this.paddleRight.width / 2), this.paddleRight.position.y + (this.paddleRight.height / 2)), this.gameMode.aiMode2);
            }
        }
        // Move paddles
    ...
    async aiMovement(side, speed, ball, paddlePos, mode) {
    ...
    handlePaddle(keyUp, keyDown, paddle) {
        if (this.keys[keyUp]) {
    ...
    handleCollision(obj, speedInertia = 0) {
