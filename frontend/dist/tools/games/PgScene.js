import { Engine, Scene, FreeCamera, Vector3, Color3, GlowLayer, StandardMaterial, MeshBuilder, ParticleSystem, Texture, } from "@babylonjs/core";
...
export default class PgScene {
    ...
    objects = {};
    particles = {};
    constructor(canvas, keys, hostPlayer) {
        this.canvas = canvas;
        this.keys = keys;
        ...
        this.game = new PgGame(this.keys, {
            update: async (positions, speed, scores, ballDirection) => {
            ...
            showResult: (leftScore, rightScore) => {
    ...
    boxObjectOffsetfrom2D(obj2D) {
        return new Vector3(obj2D.position.x + (obj2D.width ? obj2D.width : 0) / 2 + this.boxMeshOffset.x, 0.5, obj2D.position.y + (obj2D.height ? obj2D.height : 0) / 2 + this.boxMeshOffset.z);
    }
    initObjects() {
        // Balle
        const ball = {
    ...
    triggerCollisionEffect(direction) {
