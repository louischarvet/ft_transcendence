import { Vector3, StandardMaterial, Texture, Material } from "@babylonjs/core";
export default class BjCard {
    ...
    activePlaces = [];
    // Step: Move from deck
    dealStep = { position: new Vector3(-0.4, 1, 0.15), rotation: new Vector3(-Math.PI, 0, 0) };
    constructor(numberOfDeck, scene, assetsManager) {
        this.scene = scene;
    ...
    createCardMaterial(name, pattern) {
    ...
    async resetDeck() {
        ...
        console.log(`[BjCard] Shuffling ${shuffledCards.length} cards`);
    ...
    async moveCard(card, position, rotation, withoutStep = false, timeTo = 500) {
        if (!withoutStep) {
    ...
    async beginDealingCards(places) {
    ...
    discardTrayPosition = new Vector3(0, 1, 0);
    cleanPlaces() {
    ...
    async dealPlace(cardName, placeName, onSplit = false) {
